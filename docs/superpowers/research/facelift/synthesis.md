...
| **Listening: Reciter detail**                   | `reciter/[slug]/page.tsx`. `reciter-header.tsx:36-47` has stacked meta + bio. Rewayat chips as filled pills. Play All floats in its own band.                                    | Left-hero layout: 220px circular artwork + title + single pipe-meta line `{rewayat} rewayat · {surah_total} surahs · {style}` (drop bio). **Action row BELOW artwork**: Play All + ghost heart/share/kebab. Rewayat chips → underlined tab strip (they're nav, not filters). Add "Similar reciters" + "Popular surahs" rails below tracklist. Tracklist density: `surah-list-item.tsx` gets inline duration + accent-color on active row + listened-state marker. | Deezer 03                              |
